...
function HotelDetails() {
  ...
  const URL = `${import.meta.env.VITE_PRODUCTION_URL}/api/hotels/${UID}`;
  const ownerURL = `${
    import.meta.env.VITE_PRODUCTION_URL
  }/api/serviceUsers/${UID}`;
  ...
  useEffect(() => {
    const getHotel = async () => {
      try {
        await axios.get(URL).then((response) => {
          setName(response.data.hotelName);
    ...
    const getOwnerDetails = async () => {
      try {
        await axios.get(ownerURL).then((response) => {
  ...
  return (
    <>
      ...
      <div class="flex flex-col items-center -mt-40">
        <img src={photo} class="w-40 h-40 border-4 border-white rounded-full" />
